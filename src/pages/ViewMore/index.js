...
const ViewMore = () => {
  ...
  const urlSearchFilter =
    locationParams.search + `&limit=12&page=${vehiclePage}`;
  ...
  useEffect(() => {
    dispatch(listVechileAction(urlSearchFilter))
      ...
      .catch((err) => {
        if (err) {
          dispatch(listVehiclePopularAction(paramsPopulerVehicle)).catch(
            ({ ...err }) => {
              if (err) {
  ...
  return (
    <Main>
      <main className={`container ${styles["top-view-more"]}`}>
        ...
        {vehicleArr.length !== 0 ? (
          <React.Fragment>
            <p className={styles["data-vehicle"]}>{`Total vehicle ${
              meta.totalData
            } remaining vehicle ${remamagingData < 0 ? 0 : remamagingData}`}</p>
            <section className={`${styles["all-vehicle"]}`}>
              {Array.isArray(vehicleArr) &&
                vehicleArr.length > 0 &&
                vehicleArr.map((data) => (
                  <React.Fragment key={data.id}>
          ...
          <React.Fragment>
            ...
            >
              <img
                alt="avatar"
                src={vehicleNotFound}
                className={styles["vehicle-notFound"]}
              />
              <div
                style={{
                  display: "flex",
        ...
        <section className={styles["meta-flex"]} style={{ marginTop: 23 }}>
          <p>{`Page ${vehiclePage} to remaining ${
            meta.totalPage - vehiclePage
