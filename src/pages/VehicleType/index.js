...
import {
  listVehicleCarAction,
  listVehicleMotorbikeAction,
  listVehicleBikeAction,
  listVehiclePopularAction,
} from "../../redux/actions/listVehicles";

import {
  paramCarVehicle,
  paramMotorbikeVehicle,
  paramBikeVehicle,
  paramsPopulerVehicle,
} from "../../modules/helper/listVehicle";
...
const VehicleType = () => {
  ...
  const { listCarHome, listMotorbikeHome, listBikeHome, listPopularVehicle } =
    state.listVehicle;
  ...
  useEffect(() => {
    ...
    dispatch(listVehicleCarAction(paramCarVehicle)).catch((err) => {
      if (err) {
        navigate("/error%20server");
      }
    });
    dispatch(listVehicleMotorbikeAction(paramMotorbikeVehicle)).catch((err) => {
      if (err) {
        navigate("/error%20server");
      }
    });
    dispatch(listVehicleBikeAction(paramBikeVehicle)).catch((err) => {
      if (err) {
        navigate("/error%20server");
      }
    });
    ...
    <Main>
      <main className={`container ${styles["top-vehicle-type"]}`}>
        <form onSubmit={searchHandler}>
          <input
            type={"text"}
            placeholder="Search vehicle (ex. type vehicle, vehicle name, location name)"
            className={styles["search-vehicle"]}
        ...
        <section className="container">
          ...
          <div className={styles["popular-town-list"]}>
            {listPopularVehicle !== [] ? (
              <React.Fragment>
                {Array.isArray(listPopularVehicle) &&
                  ...
                  listPopularVehicle.map((data) => (
                    <React.Fragment key={data.id}>
                      <CardVehicle
                        id={data.id}
                        image={
                          data.image !== null ? data.image : vehicleImgDefault
                        }
                        name={data.name}
                      />
                    </React.Fragment>
        ...
        <section className="container">
          <div className="row justify-content-between">
            ...
            <div className="col-sm-1">
              <p className={styles["view-all"]} onClick={viewAllCarHandler}>
                View All {">"}
              </p>
            </div>
          </div>
        ...
        <section className="container">
          <div className="row justify-content-between">
            ...
            <div className="col-sm-1">
              <p
                ...
                onClick={viewAllMotorbikeHandler}
              >
                View All {">"}
              </p>
            </div>
          ...
          <div className={styles["popular-town-list"]}>
            {listMotorbikeHome !== [] ? (
              <React.Fragment>
                {Array.isArray(listMotorbikeHome) &&
        ...
        <section className="container" style={{ marginBottom: 17 }}>
          <div className="row justify-content-between">
            ...
            <div className="col-sm-1">
              <p className={styles["view-all"]} onClick={ViewAllBikeHandler}>
                View All {">"}
              </p>
            </div>
          </div>
